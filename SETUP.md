...
1. You must accept the Shopify Partner Invitation if you haven't already
...
Once you have cloned the repo, you will be able to run commands on your app via the Shopify CLI.
...
1. Bootstrap the default [SQLite](https://www.sqlite.org/index.html) database and add it to your `.env` file:
   ...
   **NOTE**: Once you create the database file, make sure to update your `DB_DATABASE` variable in `.env` since Laravel requires a full/absolute path to the file.
